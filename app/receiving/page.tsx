...
export default async function Page({searchParams}:{searchParams:Promise<{error?:string;message?:string}>}){
 ...
 const [{data:pos},{data:receipts}]=await Promise.all([
  ...
  org?supabase.from("receipts").select("id,receipt_no,status,received_at,purchase_orders(po_no),profiles:received_by(full_name)").eq("organization_id",org).order("created_at",{ascending:false}).limit(100):Promise.resolve({data:[]} as any)
 ]);
 return <AppShell title="FYNEXO Receive" subtitle="Recepción total y parcial de bienes y servicios.">
  ...
  <section className="panel"><p className="eyebrow">Pendiente</p><h2>Órdenes por recibir</h2>{(pos||[]).length?<div className="table-wrap"><table className="data-table"><thead><tr><th>PO</th><th>Proveedor</th><th>Estado</th><th></th></tr></thead><tbody>{(pos||[]).map((p:any)=><tr key={p.id}><td><b>{p.po_no}</b></td><td>{p.vendors?.legal_name||"—"}</td><td><span className="status-pill">{p.status}</span></td><td><form action={receiveRemaining}><input type="hidden" name="poId" value={p.id}/><button className="primary-button">Recibir pendiente</button></form></td></tr>)}</tbody></table></div>:<p className="muted">No hay órdenes pendientes de recepción.</p>}</section>
