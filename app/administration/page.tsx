...
export default async function Page({ searchParams }: { searchParams: Promise<{ error?: string; message?: string; invite?: string; email?: string; expires?: string }> }) {
  ...
  const [{ data: roles }, { data: members }, { data: invites }] = await Promise.all([
    ...
    organizationId ? supabase.from("organization_memberships").select("user_id,membership_role,is_active,created_at,profiles:user_id(full_name,email),user_roles!inner(role_id,roles(name,code))").eq("organization_id", organizationId).order("created_at") : Promise.resolve({ data: [] } as any),
    ...
    <AppShell title="FYNEXO Control Center" subtitle="Usuarios, roles, invitaciones y controles de acceso.">
      ...
      <div className="form-grid">
        ...
        <section className="panel span-2">
          <p className="eyebrow">Usuarios</p><h2>Membresías activas</h2>
          {(members || []).length ? <div className="table-wrap"><table className="data-table"><thead><tr><th>Usuario</th><th>Rol</th><th>Estado</th><th>Administrar</th></tr></thead><tbody>{(members || []).map((m: any) => { const profile = Array.isArray(m.profiles) ? m.profiles[0] : m.profiles; const ur = Array.isArray(m.user_roles) ? m.user_roles[0] : m.user_roles; const role = Array.isArray(ur?.roles) ? ur.roles[0] : ur?.roles; return <tr key={m.user_id}><td><b>{profile?.full_name || "Usuario"}</b><small>{profile?.email || m.user_id}</small></td><td>{role?.name || m.membership_role}</td><td><span className="status-pill">{m.is_active ? "ACTIVO" : "INACTIVO"}</span></td><td><form action={setUserRole} className="inline-form"><input type="hidden" name="userId" value={m.user_id}/><select name="roleId" defaultValue={ur?.role_id || ""}>{(roles || []).map((r: any) => <option key={r.id} value={r.id}>{r.name}</option>)}</select><select name="active" defaultValue={String(m.is_active)}><option value="true">Activo</option><option value="false">Inactivo</option></select><button className="secondary-button">Guardar</button></form></td></tr>; })}</tbody></table></div> : <p className="muted">No hay usuarios vinculados.</p>}
        </section>
      </div>
