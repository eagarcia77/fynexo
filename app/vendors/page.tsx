...
export default async function Page({ searchParams }: { searchParams: Promise<{ error?: string; message?: string }> }) {
  ...
  const { data: vendors } = organizationId
    ? await supabase.from("vendors").select("id,vendor_no,legal_name,tax_id_masked,email,phone,status,created_at").eq("organization_id", organizationId).order("legal_name")
    : { data: [] as any[] };
    ...
    <AppShell title="Vendor 360°" subtitle="Proveedores, estado, contacto y trazabilidad de compras.">
      ...
      <div className="form-grid">
        <section className="panel">
          ...
          <form action={createVendor} className="entity-form">
