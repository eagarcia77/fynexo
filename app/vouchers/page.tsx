...
export default async function Page({searchParams}:{searchParams:Promise<{error?:string;message?:string}>}){
 ...
 const [{data:matched},{data:vouchers,error:voucherError},{data:canCreate},{data:canApprove}]=await Promise.all([
  ...
  org?supabase.rpc("current_user_has_permission",{p_org:org,p_permission:"voucher.create"}):Promise.resolve({data:false} as any),
  org?supabase.rpc("current_user_has_permission",{p_org:org,p_permission:"voucher.approve"}):Promise.resolve({data:false} as any)
 ]);
