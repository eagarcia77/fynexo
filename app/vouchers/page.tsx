...
export default async function Page({searchParams}:{searchParams:Promise<{error?:string;message?:string}>}){
 ...
 const [{data:matched},{data:vouchers}]=await Promise.all([
  ...
  org?supabase.from("vouchers").select("id,voucher_no,status,total_amount,created_by,approved_at,invoices(invoice_no),profiles:created_by(full_name)").eq("organization_id",org).order("created_at",{ascending:false}).limit(100):Promise.resolve({data:[]} as any)
 ]);
